...
const allowPage = ['/', '/login', '/json']  // 待定，因为目前后端的/login都是POST
...
const localFilter = async (ctx) => {
  ...
  if(allowPage.indexOf(url) > -1) {
     ...
     */
      ...
      let refreshResult = await refreshResult(checkResult)
      /**
       ...
       * PS: 
       * 1.目前感觉累赘了，首先refreshResult()中只返回token就可以了，待修改
